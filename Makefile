...
LD_FLAGS = -X github.com/go-vela/vela-influx/version.Commit=${GITHUB_SHA} -X github.com/go-vela/vela-influx/version.Date=${BUILD_DATE} -X github.com/go-vela/vela-influx/version.Go=${GOLANG_VERSION} -X github.com/go-vela/vela-influx/version.Tag=${GITHUB_TAG}
...
build:
	@echo
	@echo "### Building release/vela-influx binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
		-ldflags '${LD_FLAGS}' \
		-o release/vela-influx \
		github.com/go-vela/vela-influx/cmd/vela-influx
...
build-static:
	@echo
	@echo "### Building static release/vela-influx binary"
	GOOS=linux CGO_ENABLED=0 \
		go build -a \
...
build-static-ci:
	@echo
	@echo "### Building CI static release/vela-influx binary"
	@go build -a \
		-ldflags '-s -w -extldflags "-static" ${LD_FLAGS}' \
		-o release/vela-influx \
		github.com/go-vela/vela-influx/cmd/vela-influx
...
docker-build:
	@echo
	@echo "### Building vela-influx:local image"
...
docker-test:
	@echo
	@echo "### Testing vela-influx:local image"
	@docker run --rm \
		vela-influx:local
...
docker-run:
	@echo
	@echo "### Executing vela-influx:local image"
	@docker run --rm \
